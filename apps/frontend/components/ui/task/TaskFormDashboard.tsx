...
export default function TaskFormDashboard({ onSubmit, loading, onClose }: TaskFormProps) {
    ...
    const teamMembers = useProjectTeamMembers(form.project_id);
    ...
    return (
        <>
            <div
                className={`fixed inset-0 flex items-start justify-center z-50 p-4 pt-20 transition-all duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'
                    }`}
                style={{
                ...
                <div className={`bg-neutral-900/95 backdrop-blur-xl border border-neutral-700/30 rounded-xl shadow-2xl w-full max-w-md mx-auto transform transition-all duration-300 ease-out ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'
                    ...
                    <form onSubmit={handleSubmit} className="p-4 space-y-4">
                        ...
                        <div className="relative" ref={projectDropdownRef}>
                            <label htmlFor="project_id" className="block text-xs text-neutral-400 mb-1">
                            ...
                            <button
                                ...
                                aria-required="true"
                                className="w-full flex items-center justify-between px-3 py-2 bg-neutral-800 border border-neutral-700 rounded text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 transition-colors"
                            >
                                <span className={!form.project_id ? "text-neutral-500" : ""}>
                                    {form.project_id
                                        ? recentProjects.find(p => p.id === form.project_id)?.name || "Select project..."
                                        : "Select project..."}
                                </span>
                                <span className="ml-2">
                            ...
                            {showProjectDropdown && (
                                <div className="absolute left-0 top-full mt-1 w-full bg-neutral-800 border border-neutral-700 rounded-lg shadow-xl z-20 max-h-60 overflow-y-auto">
                                    <div className="px-4 py-2 text-xs font-medium text-neutral-400 border-b border-neutral-700/50">
                                        Select a project
                                    </div>
                                    {recentProjects.length === 0 && (
                                        <div className="px-4 py-2 text-neutral-400 text-sm">No projects found</div>
                                    )}
                                    {recentProjects.map((project) => (
                                        <button
                                            ...
                                            className={`w-full text-left px-4 py-3 text-sm hover:bg-blue-600/10 hover:text-blue-400 transition-colors flex items-center justify-between group ${form.project_id === project.id ? "bg-blue-600/20 text-blue-400" : "text-neutral-200"
                                                }`}
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className={`w-2 h-2 rounded-full ${form.project_id === project.id ? "bg-blue-400" : "bg-neutral-600 group-hover:bg-blue-400"}`}></div>
