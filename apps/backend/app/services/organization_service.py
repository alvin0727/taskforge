...
class OrganizationService:
    ...
    async def create_personal_workspace(user_id: ObjectId, user_name: str) -> ObjectId:
        ...
        try:
            ...
            base_slug = OrganizationService.generate_organization_slug(org_name)
            ...
            raise HTTPException(status_code=500, detail=f"Failed to create personal workspace: {str(e)}")
    ...
    async def create_team_organization(
        ...
        try:
            ...
            if len(name.strip()) < 2:
                 raise HTTPException(
                    status_code=400,
                    detail={"message": "Organization name must be at least 2 characters long."}
                )
            ...
            raise HTTPException(status_code=500, detail=f"Failed to create team organization: {str(e)}")
    ...
    async def switch_active_organization(
        ...
        try:
            ...
            if not is_member:
                raise HTTPException(status_code=403, detail={"message": "User not member of organization"})
            ...
            raise HTTPException(status_code=500, detail=f"Failed to switch active organization: {str(e)}")
    ...
    async def get_user_organizations(user_id: ObjectId) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not user:
                raise HTTPException(status_code=404, detail={"message": "User not found"})
            user_orgs = user.get("organizations", [])
            organization_ids = [org["organization_id"] for org in user_orgs if org.get("status") == "active"]
            ...
            for org in organizations:
                ...
                user_org = next(
                    (uo for uo in user_orgs if uo["organization_id"] == org["_id"]), 
                    None
                )
